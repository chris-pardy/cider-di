...
describe('a singleton', () => {
  it('will return the same value with each invocation', () => {
    ...
    const injector = cider();
    const v1 = provider(injector, 'name');
    const v2 = provider(injector, 'name');
    expect(v1).toBe(v2);
  });
  ...
  it('will be different for different children', () => {
    ...
    const provider = cider.singleton(() => incrementor++);
    const injector = cider();
    const v1 = provider(injector.child(), 'name');
    const v2 = provider(injector.child(), 'name');
    expect(v1).toBe(1);
    expect(v2).toBe(2);
  });
  ...
  it('will lazy initalize', () => {
    ...
    const injector = cider();
    incrementor = 3;
    expect(provider(injector, 'name')).toBe(3);
  });
  ...
  it('can have a dependency overriden', () => {
    const injector = cider(
      cider.override(
        ...
        (bind) => {
          bind('test_value').to((inj, name) => name);
        }
      )
    );
