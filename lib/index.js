  ...
  for (var i = 0; i < extensions.length; i++) {
    ...
    for (var key in providers) {
      if(providers.hasOwnProperty(key)) {
        extension_point.providers[key] = providers[key].provider;
      }
    }
    extensions[i](extension_point);
    for (var key in extension_point.providers) {
      if (extension_point.hasOwnProperty(key)) {
        if (!providers[key]) {
          providers[key] = {}
        }
        providers[key].provider = extension_point[key];
      }
    }
    modules = extension_point.modules;
  }
